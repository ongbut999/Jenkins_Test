...
#endif
                {
					/* Aging allowed related check not placed inside Dem_EvMemIsAgingCalculationAllowed, as EventId must be passed as a parameter to this function for checking AgingAllowed.
					* But the function Dem_EvMemIsAgingCalculationAllowed may be already used with Project specific implementation, and the new function definition will not be compatible. */
