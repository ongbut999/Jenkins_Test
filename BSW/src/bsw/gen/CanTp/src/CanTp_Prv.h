...
#define CANTP_MAXFDFFDL                              (0xFFFFFFFFu)
...
#define CANTP_FDSFDL_PCI                             (1u)       /* SF_DL bytes in case of CAN-FD */
...
/* Buffer enquiry states */
#define CANTP_BUFFER_NOT_ASKED                       (0u)
#define CANTP_BUFFER_ASKED                           (1u)
#define CANTP_BUFFER_AVAILABLE                       (2u)
...
/* BitFields Mask */
#define CANTP_TRIGGER_TRANSMIT_ON                    (0x20u)
...
typedef struct
{
    uint8 TxBufferStatus;                           /* Buffer enquiry State management */
    #if(CANTP_CANFD_SUPPORT == CANTP_ON)
...
/***********************************************************************************************************************
 Function name    : CanTp_IncrementCounter
 Syntax           : CanTp_IncrementCounter()
 Description      : Library function to increment MainFunction ticks in case Cyclic timer is used.
 Parameter        : None
 Return value     : None
***********************************************************************************************************************/
...
LOCAL_INLINE void CanTp_Prv_CanIfTransmit(const CanTp_TxContextType *Context, const PduInfoType *CanIfTxInfoPtr)
{
    ...
    if(CanTp_Prv_GetBit(Context->BitFields, CANTP_TRIGGER_TRANSMIT_ON))
    {
    ...
    #endif
    {
        ...
        Result = (CanTp_Prv_CreateFrame(Context->ChannelId, &CanIfTxInfo) == E_OK) ? E_OK : Result;
    }
...
/***********************************************************************************************************************
 Function name    : CanTp_Prv_PageConfirmation
 Syntax           : CanTp_Prv_PageConfirmation(&Context)
 Description      : This is a wrapper function to update for upper layer page refilling.
 Parameter        : const CanTp_TxContextType*
 Return value     : None
***********************************************************************************************************************/
LOCAL_INLINE void CanTp_Prv_PageConfirmation(const CanTp_TxContextType *Context)
{
    PduInfoType SduInfo;
    PduLengthType RemBufSize;
    uint8 SduBuffer[CANTP_MAX_NPDU_LENGTH];
    ...
    if(CanTp_Channel[Context->ChannelId].TxBufferStatus == CANTP_BUFFER_NOT_ASKED)
    {
        ...
        /* As Page confirmation is done only in case of not availability of buffer so void typecasting is ok. */
