...
CanTp_ChannelIdType CanTp_TxConfirmationChannel[CANTP_MAX_NO_OF_TX_NPDU];
...
/***********************************************************************************************************************
 Function name    : CanTp_Prv_GetTxBuffer
 Syntax           : CanTp_Prv_GetTxBuffer(&Context, &SduInfo)
 Description      : This is a wrapper function to negotiate the Tx Buffer for FF/CF/SF Transmission.
 Parameter        : const CanTp_TxContextType*, PduInfoType*
 Return value     : None
***********************************************************************************************************************/
static void CanTp_Prv_GetTxBuffer(const CanTp_TxContextType *Context, PduInfoType* SduInfoPtr)
{
    ...
    {
        if(CanTp_Channel[Context->ChannelId].TxBufferStatus != CANTP_BUFFER_AVAILABLE)
        {
            switch(PduR_CanTpCopyTxData(Context->PduRPduHandleId, SduInfoPtr, NULL_PTR, &RemBufSize))
            {
                case BUFREQ_OK: /* Register message for transmission with CanIf */
                    ...
                    {
                        CanTp_Channel[Context->ChannelId].TxBufferStatus = CANTP_BUFFER_AVAILABLE;
                    }
#if (CANTP_PAGE_CONFIRMATION == CANTP_ON)
                    else
                    {
                        CanTp_Prv_PageConfirmation(Context);
                    }
#endif
                    break;
                case BUFREQ_E_BUSY:
                    /* TRACE[SWS_CanTp_00184][SWS_CanTp_00089] If PduR returns E_BUSY, CanTp retry to copy data. */
                    break;
		...
		#endif
		{
			...
			{  /* TRACE[SWS_CanTp_00166][SWS_CanTp_00325][SWS_CanTp_00312] */
				...
				if(RetValue == BUFREQ_OK)
				{
                    ...
                    NotifyValue = (Channel->SduLengthRemaining != 0u) ? CANTP_INVALID_NOTIFY_VALUE : E_OK;
				}
			}
...
static void CanTp_Prv_TxSendConsecutiveFrame(CanTp_ChannelIdType ChannelId)
{
	...
	if(TxConfirmationChannel == CANTP_INVALID_CHANNEL) /* It means confirmation is not pending for this Pdu. */
	{
        ...
        if((Channel->TxBufferStatus == CANTP_BUFFER_AVAILABLE) &&
           ((Context.ElapsedValue >= Channel->STminTicks) || (Channel->PciId != CANTP_NPCI_CF)))
        {
            Channel->PciId = CANTP_NPCI_CF;
            Channel->InitialTicks = Context.Value;
            CanTp_SubState[ChannelId] = CANTP_TX_WAIT_FOR_TRANSMIT_CONFIRMATION;
