...
static boolean NvM_Prv_IsBlockPartOfReadAll(NvM_BlockIdType idBlock_uo);
static void NvM_Prv_FindBlocksForReadAll(void);
...
#endif
{
    ...
    if (NvM_Prv_ErrorDetection_IsNvmInitialized(NVM_SERVICE_ID_READ_ALL, NVM_PRV_MULTI_BLOCK))
    {
        ...
        if (isRequestEnqueued_b)
        {
            ...
            NvM_Prv_Block_SetIsNvmEnqueuingForMulti(NvM_Prv_IsBlockPartOfReadAll);
...
static boolean NvM_Prv_IsBlockPartOfReadAll(NvM_BlockIdType idBlock_uo)
{
    boolean isPRamBlockValid_b = NvM_Prv_Block_IsPRamBlockValid(idBlock_uo);
    ...
    return (
            ...
            // CAUTION: After successfull Read, Write or Restore jobs, if NvM_ReadAll is invoked,
            //          the block will be skipped. This is the same if NvM_SetRAMBlockStatus
            //          was successfully invoked before NvM_ReadAll invocation.
            !isPRamBlockValid_b
           );
}
...
static void NvM_Prv_FindBlocksForReadAll(void)
{
    ...
    for (idBlock_uo = NVM_CFG_FIRST_USED_BLOCK; idBlock_uo < NVM_CFG_NR_BLOCKS; ++idBlock_uo)
    {
        ...
        {
            if (NvM_Prv_IsBlockPartOfReadAll(idBlock_uo))
            {
