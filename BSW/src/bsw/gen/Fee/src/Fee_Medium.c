    ...
    else
    {
        if(FEE_PRV_LOAD_BUSY_E != Fee_Prv_MediumLoad_st.state_en)
        {
            if(
                ...
                (length_u32              <= sizeof(Fee_Prv_MediumBuffer_un.u8)      ) && /* Request amount must fit into buffer                             */
            ...
            )
            {
                if(FEE_PRV_POSTLOAD_E == loadMode_en)
                {
                    Fee_Prv_MediumLoad_st.reloadAdr_u32 = Fee_Prv_LibMin(
                        adr_u32,
                        FEE_PRV_MEDIUM_FLASHEND - sizeof(Fee_Prv_MediumBuffer_un.u8)
                    );
                }
                else
                {
                    Fee_Prv_MediumLoad_st.reloadAdr_u32 = Fee_Prv_LibMax(
                        (adr_u32 - sizeof(Fee_Prv_MediumBuffer_un.u8)) + length_u32,
                ...
                Fee_Prv_MediumRead(Fee_Prv_MediumLoad_st.reloadAdr_u32, &Fee_Prv_MediumBuffer_un.u8[0u], sizeof(Fee_Prv_MediumBuffer_un.u8));
        ...
        if(MEMIF_JOB_PENDING != result_en)
        {
            ...
            if(MEMIF_JOB_OK == result_en)
            {
                ...
                Fee_Prv_Medium_st.limitHi_u32 = Fee_Prv_MediumLoad_st.reloadAdr_u32 + sizeof(Fee_Prv_MediumBuffer_un.u8);
            }
        }
    }
...
# if (defined(FLS_RB_BLANK_CHECK_API) && (STD_OFF != FLS_RB_BLANK_CHECK_API))
    FEE_PRV_LIBTRANS(Fee_Prv_MediumWrite_st, FEE_PRV_MEDIUM_WRITE_BLANKCHECK_E);
# else
    FEE_PRV_LIBTRANS(Fee_Prv_MediumWrite_st, FEE_PRV_MEDIUM_WRITE_PROGRAM_E);
# endif
}
...
# else
    ...
    /*
     * If the Fls doesn't provide a blank check, we don't trigger it in function Fee_Prv_MediumWrite (i.e. state
     * FEE_PRV_MEDIUM_WRITE_BLANKCHECK_E is skipped). Instead, the programming is executed directly (i.e. state
     * FEE_PRV_MEDIUM_WRITE_PROGRAM_E is chosen). Therefore, Fee_Prv_MediumBlankCheck won't be called. If called,
     * something is completely wrong, and E_NOT_OK is returned to indicate the problem in the control flow.
     */
...
void Fee_Prv_MediumShredder(uint32 adr_u32, uint32 length_u32)
{
    if(length_u32 <= sizeof(Fee_Prv_MediumBuffer_un.u8))
    {
        Fee_Prv_MediumSetBfrDirty();
        /* MR12 DIR 1.1 VIOLATION: uint8 * can always be converted safely to void * */
        Fee_Prv_LibMemSet(&Fee_Prv_MediumBuffer_un.u8[0], (uint8)(~FEE_PRV_CFG_ERASE_PATTERN_8BIT), length_u32);
...
/**
 * \brief   Start a copy operation from one flash location to another (using internal buffer).
 *
 * \param   adrSrc_u32  Source address in flash
 * \param   adrDest_u32 Target address in flash
 * \param   length_u32  Bytes to copy in total
*/
void Fee_Prv_MediumCopy(uint32 adrSrc_u32, uint32 adrDest_u32, uint32 length_u32)
{
    ...
    FEE_PRV_LIBTRANS(Fee_Prv_MediumCopy_st, FEE_PRV_MEDIUM_COPY_READ_E);
}
...
static MemIf_JobResultType Fee_Prv_MediumWrite_Program(Fee_Prv_MediumWrite_tst * fsm_pst)
{
    ...
    if(FEE_PRV_LIBENTRY)
    {
        ...
        /* If necessary, copy user data to internal buffer */
        if(startAdr_pcu8 != &Fee_Prv_MediumBuffer_un.u8[0u])
        {
            /* MR12 DIR 1.1 VIOLATION: uint8 * can always be converted safely to void * */
            Fee_Prv_LibMemCopy(startAdr_pcu8, &Fee_Prv_MediumBuffer_un.u8[0u], fsm_pst->bytes2Program_u32);
        }
        else
        {
            /* Fee_Prv_MediumWrite was called from Fee_Prv_MediumCopy_Write, where the data to program is already
             * stored inside Fee_Prv_MediumBuffer_un. I.e. we can/should skip the call of Fee_Prv_LibMemCopy. */
        }

        Fee_Prv_MediumSetBfrDirty();

        /* MR12 DIR 1.1 VIOLATION: uint8 * can always be converted safely to void * */
        Fee_Prv_MediumProgram(fsm_pst->adr_u32 + fsm_pst->done_u32, &Fee_Prv_MediumBuffer_un.u8[0u], fsm_pst->bytes2Program_u32);
    }
...
static MemIf_JobResultType Fee_Prv_MediumWrite_Compare(Fee_Prv_MediumWrite_tst * fsm_pst)
{
    ...
    {
        /* MR12 DIR 1.1 VIOLATION: uint8 * can always be converted safely to void * */
        Fee_Prv_MediumCompare(fsm_pst->adr_u32 + fsm_pst->done_u32, &Fee_Prv_MediumBuffer_un.u8[0u], fsm_pst->bytes2Program_u32);
    }
...
MemIf_JobResultType Fee_Prv_MediumCalcCrcDo(void)
{
    ...
    uint32 nrBytesToConsider_u32            = Fee_Prv_LibMin(
        sizeof(Fee_Prv_MediumBuffer_un.u8),
...
*/
static MemIf_JobResultType Fee_Prv_MediumCopy_Read(Fee_Prv_MediumCopy_tst * fsm_pst)
{
    MemIf_JobResultType result_en;
...
*/
static MemIf_JobResultType Fee_Prv_MediumCopy_Write(Fee_Prv_MediumCopy_tst * fsm_pst)
{
    MemIf_JobResultType result_en;
    ...
    {
        /* MR12 DIR 1.1 VIOLATION: uint8 * can always be converted safely to void * */
        Fee_Prv_MediumWrite(fsm_pst->adrDest_u32 + fsm_pst->done_u32, &Fee_Prv_MediumBuffer_un.u8[0u], fsm_pst->bytes2Copy_u32);
    }
...
MemIf_JobResultType Fee_Prv_MediumNext(uint32 length_u32, void const * * ptr_cppcv)
{
    ...
    if(MEMIF_JOB_OK == result_en)
    {
        ...
        /* MR12 DIR 1.1, 18.6 VIOLATION: uint8 * can always be converted safely to void *, returns the pointer to a global buffer at a specific offset */
        *ptr_cppcv                      = &Fee_Prv_MediumBuffer_un.u8[index_u32];
