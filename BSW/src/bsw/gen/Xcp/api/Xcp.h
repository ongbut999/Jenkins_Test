...
/* 64bit Check: If 64bit architecture is detected, StaticAddressTransformation has to be enabled */
#if ((CPU_TYPE != CPU_TYPE_32) && (XCP_STATIC_ADDRESS_TRANSFORMATION == STD_OFF))
# error "64bit Architecture detected but StaticAddressTransformation is not enabled - this is not supported!"
#endif
...
/* Macro Xcp_GetState points to Xcp_GetStateForCalSup. The original Xcp_GetState function is renamed to Xcp_GetStateForCalSup and is deprecated.
 * The Macro will be retained only for the next two main deliveries and then be removed  */
#define Xcp_GetState Xcp_GetStateForCalSup
