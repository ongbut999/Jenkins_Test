...
// #define XCP_TESTINJECTION  STD_ON

#if defined(XCP_TESTINJECTION) && (XCP_TESTINJECTION == STD_ON)
#include "Xcp_TestInjection.h"
#else
#define XCP_TESTINJECTION_FUNC(x)
#endif
 ...
 *  \{ */
#define XCP_8BYTE_ALIGN(var)      ((((var) + 7u) / 8u) * 8u)
#define XCP_4BYTE_ALIGN(var)      ((((var) + 3u) / 4u) * 4u)
#define XCP_2BYTE_ALIGN(var)      ((((var) + 1u) / 2u) * 2u)

/* Machine alignment: Used to align to the width of an address */
#if (XCP_ADDR_LEN == 8)
# define XCP_MACHINE_ALIGN(var) XCP_8BYTE_ALIGN(var)
#else
# define XCP_MACHINE_ALIGN(var) XCP_4BYTE_ALIGN(var)
#endif
/** \} */
...
/** \name Macro for accessing global command buffer
 *  \{ */
#define XCP_CMD_BUFFER_PTR(a)     Xcp_Cleared[a].CmdBuffer.Buffer_au8
#define XCP_CMD_BUFFER_LENGTH(a)  Xcp_Cleared[a].CmdBuffer.Length_u32
/** \} */
...
#define XCP_SIZEOF_ODT_ENTRY      (sizeof(Xcp_AddrValue) + 1u + 1u) /* Address, Extension, Size */
#else
#define XCP_SIZEOF_ODT_ENTRY      (sizeof(Xcp_AddrValue) + 1u)      /* Address, Size */
#endif
/** \} */
...
#define XCP_MAXDTO_ALIGNED(a)                               Xcp_NoInit[a].Session.MaxDtoAligned_u16
/** \} */
...
/** \name OdtEntry
 *  \{ */
#define XCP_ODTENTRY_ADDRESS(AbsOdtEntryNum, protLayerId)   (Xcp_NoInit[(protLayerId)].DaqConfig.OdtEntryAddress_p[(AbsOdtEntryNum)])
#if (XCP_ADDRESS_EXTENSION == STD_ON)
#define XCP_ODTENTRY_EXTENSION(AbsOdtEntryNum, protLayerId) (Xcp_NoInit[(protLayerId)].DaqConfig.OdtEntryExtension_p[(AbsOdtEntryNum)])
#endif
...
typedef enum
{
  ...
  XCP_BG_REPEAT_CMD,                                        /**< Last command shall be repeated as it was not finished yet (pending) */
  XCP_BG_DO_DISCONNECT,                                     /**< Stop all DaqLists and then disconnect the PL */
...
typedef struct
{
  ...
  Xcp_AddrType_t  Mta;                                      /**< ASAM: MTA (Memory Transfer Address) */
  uint16          MaxDto_u16;                               /**< ASAM: MAX_DTO, copy from const */
  uint16          MaxDtoAligned_u16;                        /**< MAX_DTO aligned to 4/8 byte */
...
typedef struct
{
  uint8*  GetIdAddrPtr;                                     /**< GetId Pointer */
...
typedef struct
{
  uint8*          DaqListQue_p;                             /**< DaqListQue start pointer (aligned to uint32/uint64) */
...
# endif
  ...
  volatile uint8  Mode_u8;                                  /**< Daq list mode. Can be changed in ISR by master, must be refetched every time */
  volatile boolean CurrentlyRunning_b;                      /**< Flag to indicate if DaqList is currently running (e.g. sampling data). Can be changed in ISR by master, must be refetched every time */
}Xcp_DaqList_t;
...
typedef struct
{
  ...
  Xcp_AddrValue*  OdtEntryAddress_p;                        /**< OdtEntryAdress array start pointer */
#if (XCP_ADDRESS_EXTENSION == STD_ON)
  uint8*          OdtEntryExtension_p;                      /**< OdtEntryExtension array start pointer */
#endif
  uint8*          OdtEntrySize_p;                           /**< OdtEntrySize array start pointer */
  uint16*         PriorityList_p;                           /**< PriorityList array start pointer */
  uint8*          DaqQue_p;                                 /**< DaqQue start pointer (aligned to uint32/uint64) */
...
# endif
  ...
  /* Daq ram variables */
  uint8*          DaqRamPtr_pu8;                            /**< Start of Daq ram (aligned to uint32/uint64), same as DaqList_p */
...
typedef struct
{
  ...
  Xcp_ErrorCode     BgDoDisconnect_Response;                /**< Response that shall be send by the BG-Activity: DoDisconnect */
  ...
  Xcp_Cto8_t        CmdBuffer;                              /**< Command buffer of size 8 with command length */
#if (XCP_BLOCK_TRANSFER_UPLOAD == STD_ON || XCP_BLOCK_TRANSFER_DOWNLOAD == STD_ON || XCP_UPLOAD_DOWNLOAD_MEMORY_ACCESS_BY_APPL == STD_ON)
...
extern Xcp_ErrorCode  Xcp_DaqSetOdtCopyRoutine(const Xcp_AddrType_t XcpAddr, uint8 Size, uint16 AbsOdtNum, uint8 protLayerId);
# endif
...
#define Xcp_MemCopy(  xDest_pv,   xSrc_pcv,     numBytes_u32) (void)rba_BswSrv_MemCopy(  (void*)(xDest_pv),    (const void*)(xSrc_pcv),     (numBytes_u32))
...
#define Xcp_MemSet(   xDest_pv,   xPattern_s32, numBytes_u32) (void)rba_BswSrv_MemSet(   (void*)(xDest_pv),    (xPattern_s32), (numBytes_u32))
...
extern void Xcp_Cmd0xC0Handling             (const PduInfoType* XcpPacket, uint8 protLayerId);
#define XCP_CMD_0XC0_PFCT                   (Xcp_CmdFunctionPtr)(&Xcp_Cmd0xC0Handling)
#else
#define XCP_CMD_0XC0_PFCT                   (Xcp_CmdFunctionPtr)(NULL_PTR)
#endif

/** \} */


/* * \name Sub-command functions
 *  \{ */
...
\return     -
***************************************************************************************************/
extern void Xcp_SubCmdGetVersion            (const PduInfoType* XcpPacket, uint8 protLayerId );

#endif /* XCP_SUBCMD_0XC0_GET_VERSION_AVAILABLE == STD_ON */
