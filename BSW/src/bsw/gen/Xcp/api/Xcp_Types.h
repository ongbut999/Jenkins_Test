 ...
 *   | | | | | | +--- Optimisation_Type_1
 *   | | | | | +----- Optimisation_Type_2
 *   | | | | +------- Optimisation_Type_3 (specified in Xcp_OdtOptimizationType_t)
 ...
 *   +--------------- Identification_Field_Type_1  - 10: Relative ODT number, absolute DAQ list number (WORD), 11: Relative ODT number, absolute DAQ list number (WORD, aligned) (specified in Xcp_IdField_t)
 * ```
 * \{ */
#define XCP_DAQ_KEY_BYTE_ADDRESS_EXTENSION_FREE        0x00u /**< Address extension is not restricted */
#define XCP_DAQ_KEY_BYTE_ADDRESS_EXTENSION_ODT         0x10u /**< Address extension shall be the same within one ODT */
#define XCP_DAQ_KEY_BYTE_ADDRESS_EXTENSION_NOT_ALLOWED 0x20u /**< Address extension is not allowed */
#define XCP_DAQ_KEY_BYTE_ADDRESS_EXTENSION_DAQ         0x30u /**< Address extension shall be the same within one DAQ (and therefore within an ODT) */
/** \} */
...
/** \name Initialization status of XCP
 *  \{ */
...
/** \name Xcp_GetStatus
 *  \{ */
#define XCP_DAQ_IS_ACTIVE      0x01u
#define XCP_STIM_IS_ACTIVE     0x02u
/** \} */
...
/* Find out if we are on 32bit or 64bit.
 * This has to be done as with XcpOnPosix it is possible to run on 64bit-Systems.
 * As the RTA-BSW defines are also available in XcpOnPosix, there CPU_TYPE is extended to support 64bit.
 * If the 64bit-constant is available we can check if it is also set as CPU_TYPE and define the variables accordingly
 */
#ifndef CPU_TYPE_64
/* 64bit-constant is not available - this means we are on RTA-BSW -> 32bit-System */
# define XCP_ADDR_TYPE uint32
# define XCP_ADDR_LEN  4u
#else
/* 64bit-constant is available: Check if we are compiled for 64bit */
# if (CPU_TYPE == CPU_TYPE_64)
/* This is a 64bit-System, so use the according datatypes for the addresses */
#  define XCP_ADDR_TYPE uint64
#  define XCP_ADDR_LEN  8u
# else
/* This is not 64bit. Then use 32bit */
#  define XCP_ADDR_TYPE uint32
#  define XCP_ADDR_LEN  4u
# endif
#endif
  ...
  /* Address pointers */
typedef XCP_ADDR_TYPE Xcp_AddrValue;

/** Structure to hold the address and extension that form together the location that shall be accessed */
typedef struct
{
  Xcp_AddrValue AddrValue;  /**< Address that shall be accessed */
  uint8         Extension;  /**< Address extension of the address to be accessed */
} Xcp_AddrType_t;
...
typedef enum
{
  ...
  XCP_REPEAT_COMMAND                    = 0xFCu,  /**< Internal usage - command cannot be processed, will be called later again */
...
/** Xcp connection, DAQ and STIM states*/
typedef struct
{
...
typedef enum
{
  ...
  XCP_UTILS_SID                         = 0x58u,  /**< Service Id for Xcp_Utils */
