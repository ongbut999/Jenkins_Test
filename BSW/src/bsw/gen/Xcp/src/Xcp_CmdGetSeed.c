...
void Xcp_CmdGetSeed(const PduInfoType* XcpPacket, uint8 protLayerId)
{
  ...
  else if(CmdPtr->Mode_u8 == 0x01)
  {
    ...
    if (Xcp_Cleared[protLayerId].SeedAndKey.SeedRemaingSize_u8 > 0u)
    {
      ...
      SeedPtr = (uint8 *)XCP_MTA(protLayerId).AddrValue;
    }
    else
    {
  ...
  if (Error == XCP_NO_ERROR)
  {
    ...
    XCP_MTA(protLayerId).AddrValue = (Xcp_AddrValue)SeedPtr;
    XCP_MTA(protLayerId).Extension = 0u; /* 0 is direct memory access - and also is not a bad choice if AddrExtension is disabled */
  }
  else
  {
