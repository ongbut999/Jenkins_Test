...
static Xcp_ErrorCode Xcp_PrepareNVMBlock(uint8 protLayerId)
{
  ...
  else
  {
    ...
    WriteRamPos = XCP_MACHINE_ALIGN((XCP_ADDR_TYPE) Xcp_NoInit[protLayerId].DaqConfig.PriorityList_p) - (XCP_ADDR_TYPE) Xcp_NoInit[protLayerId].DaqConfig.DaqRamPtr_pu8;
...
static void Xcp_ConnectTransportLayer(uint8 protLayerId, uint8 XcpTransportLayerId)
{
  ...
  Xcp_NoInit[protLayerId].Session.MaxDtoAligned_u16 = XCP_MACHINE_ALIGN(Xcp_NoInit[protLayerId].Session.MaxDto_u16);
