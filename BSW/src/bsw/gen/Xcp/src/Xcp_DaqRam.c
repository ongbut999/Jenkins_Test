...
#  endif /*  (XCP_DAQRAM_SHIFTING == STD_ON) */
    ...
    /* MR12 RULE 11.5, 1.1 VIOLATION: The calling layers ensure correct alignment.
...
boolean Xcp_DaqRamTriggerMovePL(const PduInfoType* XcpPacket, uint8 protLayerId)
{
    ...
    )
  {
    ...
    Xcp_GlobalNoInit.DaqRamMove.moveSize_u32 = XCP_MACHINE_ALIGN((XCP_ADDR_TYPE) Xcp_NoInit[protLayerId].DaqConfig.PriorityList_p) - (XCP_ADDR_TYPE) Xcp_NoInit[protLayerId].DaqConfig.DaqRamPtr_pu8;
...
static void Xcp_DaqRamReturn(void)
{
  ...
  /* call Xcp_DaqRamCalc to reset daq, odt and odtentry pointers, return value not needed */
