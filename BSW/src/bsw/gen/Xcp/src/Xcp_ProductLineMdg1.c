...
Xcp_ErrorCode XcpAppl_BuildChecksumTrigger(const Xcp_AddrType_t XcpAddr, uint32 BlockSize, uint8 ProtocolLayerId)
{
  ...
  {
    /* Store data for background checksum calculation (Extension not used in MDG1 so not copied) */
...
Xcp_ErrorCode XcpAppl_BuildChecksumMainFunction(uint32* ChecksumPtr, uint8* ChecksumType, uint8 ProtocolLayerId)
{
  ...
  if (XcpAppl_BuildChecksumData[ProtocolLayerId].BlockSize_u32 > 0u)
  {
    ...
    if (XcpAppl_BuildChecksumData[ProtocolLayerId].XcpAddress != 0)
    {
      ...
      DmeStatus = rba_Dme_BuildChecksumCrc32((uint32)XcpAppl_BuildChecksumData[ProtocolLayerId].XcpAddress,
      ...
      XcpAppl_BuildChecksumData[ProtocolLayerId].XcpAddress = 0;
    }
...
Xcp_ErrorCode XcpAppl_MemRead(uint8* AddrPtrDst, const Xcp_AddrType_t XcpAddrSrc, uint8 Length, uint8 ProtocolLayerId)
{
  ...
  DmeStatus = rba_Dme_Read(XcpAddrSrc.AddrValue, XcpAppl_Data[ProtocolLayerId].CalPage_u8, Length, AddrPtrDst, XcpAppl_DmeUser[ProtocolLayerId]);
...
Xcp_ErrorCode XcpAppl_MemWrite(Xcp_AddrType_t XcpAddrDst, const uint8* AddrPtrSrc, uint8 Length, uint8 ProtocolLayerId)
{
  ...
  DmeStatus = rba_Dme_Write(XcpAddrDst.AddrValue, XcpAppl_Data[ProtocolLayerId].CalPage_u8, Length, (uint8*)AddrPtrSrc, XcpAppl_DmeUser[ProtocolLayerId]);
