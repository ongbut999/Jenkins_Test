...
void Xcp_CmdWriteDaq(const PduInfoType* XcpPacket, uint8 protLayerId)
{
  ...
  else
  {
    ...
    Error = Xcp_DaqWriteDaq(CmdPtr->Address_u32, CmdPtr->AddrExtension_u8, CmdPtr->Size_u8, protLayerId);
  }
...
\param[in]  Extension     ODT entry address extension
...
***************************************************************************************************/
Xcp_ErrorCode Xcp_DaqWriteDaq(uint32 Address, uint8 Extension, uint8 Size, uint8 protLayerId)
{
  /* Local variables */
  Xcp_ErrorCode Error;
  ...
  Error = XCP_NO_ERROR;
  LocalAddress.AddrValue = (Xcp_AddrValue)Address;
  LocalAddress.Extension = Extension;
  ...
  else if (LocalAddress.Extension != 0)
  {
  ...
  else
  {
    ...
    { /* Address-Transformation for the complete DAQ has to be done here */
      Error = Xcp_DaqTransformAndCheckOdtEntry(&LocalAddress, Size, CurrentDaqListNum, protLayerId);
    }
...
# endif /* XCP_OM_ODT_OPTIMIZATION_ENABLED == STD_ON */
    ...
    if (Error == XCP_NO_ERROR)
    {
      uint16 OdtEntryPos = Xcp_NoInit[protLayerId].DaqConfig.SelectedOdtEntry.OdtEntryPos_u16;
      XCP_ODTENTRY_ADDRESS(OdtEntryPos, protLayerId)    = LocalAddress.AddrValue;
#if (XCP_ADDRESS_EXTENSION == STD_ON)
      XCP_ODTENTRY_EXTENSION(OdtEntryPos, protLayerId)  = LocalAddress.Extension;
#endif
      XCP_ODTENTRY_SIZE(OdtEntryPos, protLayerId)       = Size;
