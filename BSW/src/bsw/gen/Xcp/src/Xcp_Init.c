...
void Xcp_Init(const Xcp_ConfigType* Xcp_ConfigPtr)
{
  ...
  /* MR12 RULE 14.3 VIOLATION: The condition will only be true in case of error. To have a simple DET-Reporting function this condition is encapsulated into a macro and cannot be left out */
...
# else
  /* 64bit - A2L is currently always generated for 32bit, so skip the check */
# endif
#endif

  /* NOINIT section not used (most cases): Initialization is always done */
  /* NOINIT section used (special cases):  Initialization is done for first startup */
...
# endif
  ...
  if(Xcp_GlobalNoInit.InitStatus_u8 != XCP_INIT)
  {
