...
void Xcp_CmdReadDaq(const PduInfoType* XcpPacket, uint8 protLayerId)
{
  ...
  if (Error == XCP_NO_ERROR)
  {
    ...
    ResPtr->AddrExtension_u8 = XCP_ODTENTRY_EXTENSION(Xcp_NoInit[protLayerId].DaqConfig.SelectedOdtEntry.OdtEntryPos_u16, protLayerId);
#else
    ResPtr->AddrExtension_u8 = 0x0u;
#endif
    /* address of DAQ element */
    ResPtr->Address_u32 = XCP_ODTENTRY_ADDRESS(Xcp_NoInit[protLayerId].DaqConfig.SelectedOdtEntry.OdtEntryPos_u16, protLayerId);
