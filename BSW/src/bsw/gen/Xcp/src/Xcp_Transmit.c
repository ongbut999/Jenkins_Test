...
static void Xcp_SendDaqTxConf(Xcp_DaqList_t* DaqListPtr, uint8 protLayerId)
{
  ...
  XCP_TESTINJECTION_FUNC(XCP_TESTINJECTION_POS_TXCONF_DAQ)
  ...
  XcpPacket.SduDataPtr = &(DaqListPtr->DaqListQue_p[ReadPos * XCP_MAXDTO_ALIGNED(protLayerId)]);
...
void Xcp_SendDaq(uint16 daqListNo_u16, uint8 protLayerId)
{
  ...
  XCP_TESTINJECTION_FUNC(XCP_TESTINJECTION_POS_SEND_DAQ)
...
static Std_ReturnType Xcp_DaqQueFindNextOdtToSend(Xcp_DaqList_t** DaqListPtr, uint8 protLayerId)
{
  ...
  XCP_TESTINJECTION_FUNC(XCP_TESTINJECTION_POS_FIND_DAQ)
