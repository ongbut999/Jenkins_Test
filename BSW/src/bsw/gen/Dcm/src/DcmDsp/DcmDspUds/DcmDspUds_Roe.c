        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_INTERN_DATA) dataNegRespCode_u8)
{
    ...
    VAR(boolean, AUTOMATIC) dataStoreEventInfo_b; /*Local variable to chk if event of Did in the loop was either
                                                    started/stopped/cleared*/
    ...
    *dataNegRespCode_u8 =0x0u;
    ...
    for(;((dataidxLoop_u8<DCM_MAX_DIDROEEVENTS) && (*dataNegRespCode_u8 == 0x0u));dataidxLoop_u8++)
    {
        ...
        if((dataStoreEventInfo_b != FALSE) ||(Dcm_stDspRoeDidOpStatusIsPending_b != FALSE))
        {
            ...
            /*inform application regarding the status change of the event and store the information in the NVM
              via application*/
 ...
 * This function is used to provide relevant information to the application
 *
 * \parameters           dataReturnVal_u8 : The return type of DcmAppl_DcmStoreRoeDidInfo() is E_OK
 *                                          or DCM_E_PENDING or E_NOT_OK.
 *                       dataNegRespCode_u8 : Parameter to update the NRC value
 * \return value         Std_ReturnType :   E_OK: Storing the ROE DTC Info is successful
 *                                          DCM_E_PENDING: Need more time to store the ROE DTC Info
 *                                          E_NOT_OK: Storing the ROE DTC Info is unsuccessful
...
static FUNC(Std_ReturnType,DCM_CODE) Dcm_ControlRoeDtcEventType(
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_INTERN_DATA) dataNegRespCode_u8)
{
    VAR(boolean, AUTOMATIC) dataDtcStatusflag_b = FALSE; /*Local variable to chk if event of Did in the loop was
                                                           either started/stopped/cleared*/
    VAR(Dcm_SesCtrlType,AUTOMATIC) dataCurrentSession_u8;
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_INTERN_DATA) dataNegRespCode_u8)
{
    ...
    {
        /* Call Dcm_CheckRoeEventType() to check if the is Event Type is Start/Stop/Clear event or
           Setup event or ReportActivatedEvents*/
        Dcm_CheckRoeEventType();
    }
    else
 ...
 * This function is used to check if the control request leads to NRC 0x22 or 0x24 when validated
 * against the existing states of the roe events.
 *
 *
 * \parameters           dataNegRespCode_u8 : Parameter to update the NRC value
 * \return value
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_INTERN_DATA) dataNegRespCode_u8)
{
    ...
    {
        /*multiple client handling,atleast one of the event is started already and if the new start request came
          from a different protocol then the request shall be rejected with NRC 0x22*/
#if(DCM_CFG_DSP_ROEONDTCSTATUSCHANGE_ENABLED != DCM_CFG_OFF)
    ...
    /*checking request sequence error ;if start or stop is requested and if all the events are in the cleared state
      NRC ox24 needs to be sent*/
...
/**
 **************************************************************************************************
 * Dcm_Prv_DcmRoe_Controlevent : The ResponseOnEvent service requests a server to start stop or clear
 * transmission of responses on a Control event and setting up of events.
...
static FUNC(Std_ReturnType,DCM_CODE) Dcm_Prv_DcmRoe_Controlevent (
...
/**
 **************************************************************************************************
 * Dcm_Prv_DcmRoe_SetUpevent_WindowTime : The ResponseOnEvent service requests a server to start stop or clear
 * transmission of responses on a SetUp event and Event Window Time and setting up of events.
 * This service provides the possibility of automatically executing a diagnostic service in the
 * event that a specified event occurs in the server.
 *
 * \parameters           dataNegRespCode_u8 : Parameter to update the NRC value
...
static FUNC(Std_ReturnType,DCM_CODE) Dcm_Prv_DcmRoe_SetUpevent_WindowTime (
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_INTERN_DATA) dataNegRespCode_u8)
{
            ...
            (Dcm_stRoeEventWindowTime_u8 == DCM_ROE_EVENTWINDOWTIME_CURRENT_AND_FOLLOWING_CYCLE))
    {
        ...
        {
            /*Check if the storage state is set for event window time any other than INFINITY & CURRENT_FOLLOWING
              cycles*/
...
/**
 **************************************************************************************************
 * Dcm_Prv_DcmRoe_SetUpevent_ONCHANGEOFDID : The ResponseOnEvent service requests a server to start stop or clear
 * transmission of responses on a SetUp event and setting up of events.
 * This service provides the possibility of automatically executing a diagnostic service in the
 * event that a specified event occurs in the server.
 *
 * \parameters           pMsgContext: Pointer to message structure
...
static FUNC(Std_ReturnType,DCM_CODE) Dcm_Prv_DcmRoe_SetUpevent_ONCHANGEOFDID (
        /* MR12 RULE 8.13 VIOLATION: The pointers cannot be made constants since they are being modified in other configurtions */
                                      P2VAR(Dcm_MsgContextType,AUTOMATIC,DCM_INTERN_DATA) pMsgContext,
                                      P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_INTERN_DATA) dataNegRespCode_u8)
{
    VAR(Std_ReturnType, AUTOMATIC)   dataReturnVal_u8;             /* Local variable to store return from DcmAppl API */
...
/**
 **************************************************************************************************
 * Dcm_Prv_DcmRoe_SetUpevent_ONDTCSTATUSCHANGE : The ResponseOnEvent service requests a server to start stop or clear
 * transmission of responses on a SetUp event and setting up of events.
 * This service provides the possibility of automatically executing a diagnostic service in the
 * event that a specified event occurs in the server.
 *
...
static FUNC(Std_ReturnType,DCM_CODE) Dcm_Prv_DcmRoe_SetUpevent_ONDTCSTATUSCHANGE (
                                     P2VAR(Dcm_MsgContextType,AUTOMATIC,DCM_INTERN_DATA) pMsgContext,
                                     P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_INTERN_DATA) dataNegRespCode_u8)
{
    VAR(Std_ReturnType, AUTOMATIC) dataReturnVal_u8; /* Local variable to store return from DcmAppl API */
...
/**
 **************************************************************************************************
 * Dcm_Prv_DcmRoe_SetUpevent : The ResponseOnEvent service requests a server to start stop or clear
 * transmission of responses on a SetUp event and setting up of events.
 * This service provides the possibility of automatically executing a diagnostic service in the
 * event that a specified event occurs in the server.
 *
...
static FUNC(Std_ReturnType,DCM_CODE) Dcm_Prv_DcmRoe_SetUpevent (
        P2VAR(Dcm_MsgContextType,AUTOMATIC,DCM_INTERN_DATA) pMsgContext,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_INTERN_DATA) dataNegRespCode_u8)
{
    VAR(Std_ReturnType, AUTOMATIC) dataReturnVal_u8; /* Local variable to store return from DcmAppl API */
    ...
    if((*dataNegRespCode_u8 == 0u) && (Dcm_stRoeEventWindowTime_u8 != 0u) )
    {
        ...
        {
            *dataNegRespCode_u8 = Dcm_Prv_DcmRoe_SetUpevent_WindowTime(dataNegRespCode_u8);
        }
        ...
        if(*dataNegRespCode_u8 == 0u)
        {
            ...
            if(Dcm_stEventType_u8 == DCM_ROE_ONCHANGEOFDID)
            {
                dataReturnVal_u8 = Dcm_Prv_DcmRoe_SetUpevent_ONCHANGEOFDID(pMsgContext,dataNegRespCode_u8);
            }
            else
            {
                if(Dcm_stEventType_u8 == DCM_ROE_ONDTCSTATUSCHANGE)
                {
                    dataReturnVal_u8 = Dcm_Prv_DcmRoe_SetUpevent_ONDTCSTATUSCHANGE(pMsgContext,dataNegRespCode_u8);
                }
            }
        }
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_INTERN_DATA) dataNegRespCode_u8)
{
    ...
    *dataNegRespCode_u8 = 0x0u; /* Local variable initialisation */
    ...
    else
    {
        ...
        {
            dataReturnVal_u8 = Dcm_Prv_DcmRoe_Controlevent(pMsgContext,dataNegRespCode_u8);
        }
        /* Event Type is OnDTCStatusChange or OnChangeOfDataIdentifier */
        if(Dcm_stDspRoeState_en == DCM_DSP_ROE_SETUPEVENT)
        {
            dataReturnVal_u8 = Dcm_Prv_DcmRoe_SetUpevent(pMsgContext,dataNegRespCode_u8);
        }
