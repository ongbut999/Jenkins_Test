...
elsif ($map_type == 4) #Tasking
{
	$map_file_regex= "[\|]\\s+([a-zA-Z0-9]{1,}+[\:]+[a-zA-Z0-9]{1,})\\s+[\|]\\s+([a-zA-Z0-9]{1,}\\w+).+(0x[0-9a-fA-F]{1,}).+(0x[0-9a-fA-F]{1,}).+(0x[0-9a-fA-F]{1,}).+(0x[0-9a-fA-F]{1,})";#\\s+[\|]+([a-zA-Z0-9]{1,}\\w+)";
	#                   |                     mpe:pflash0                 |  PFLASH0_CALIB     | 0x00004000 | 0x801c0000 | 0x001c0000 | 0x00000004 |";
#0x1000000   0x580000    373936( 6%)     0x270  flash_memory
#0x8f90000    0x10000        40( 0%)         0  mirrorfls
}
else
  ...
  while(<MAP>)
  {
	if( s/$map_file_regex//)
	{
		...
		if (grep( /$segment/, @replacement_list ))
		{
			...
			if ($lastsegment eq $segment)
			{
				$segment_size{$segment}="0x".sprintf ("%08x",(hex($size) + hex($sizetemp))); 
				$segment_addr{$segment} = $lastaddress;
			}
			else
			{
