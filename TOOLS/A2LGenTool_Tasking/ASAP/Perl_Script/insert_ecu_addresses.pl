...
my %Supported_MAP_Types = (
	...
	"TASKING_TC27X"	=>	"Tasking format for TC27X"	
    );
...
my %MAP_Pattern_Type =(
	...
	"TASKING_TC27X"	=> "Format_TASKING_TC27x"
    );
...
my %REG_EXP_Patterns =(
	...
	"Format_TASKING_TC27x"		=> "[\|]\\s+([a-zA-Z0-9]{1,}\\w+)\\s+[\|]\\s+(0x[0-9a-fA-F]{8})\\s+[\|]\\s+[\|]\\s" #+ ([a-zA-Z0-9]{1,}\\w+)\\s + [\W\\s] + 0x([0-9a-fA-F]{8})\\w+ "  
);	
   ...
   while (<MAP>)  # Read Line of Input File 
   {
     ...
     while ( s/$REG_EXP_Use// )
     {
		...
		elsif ("Format_Power_MPC574"  eq $MAP_Pattern_Type{$MAP_Type} )
		{
			 ...
			 if ($DODEBUG_OUT == 1)
			 {
				print "Match (MPC5748G) found: Name:$varname  Address:$var_Address  Size:$var_Size  \n\n";
			 }
			 ######################################################
			 # Check if Element is in List to find
			 ######################################################
		...
		elsif ("Format_TASKING_TC27x"  eq $MAP_Pattern_Type{$MAP_Type} )
		{
			 ######################################################
			 # Set the variables from the corresponding Search pattern results
			 ######################################################
               ...
               || s/$RegExp_Size_Calc// ) 
        {
           ...
           if ( ($1 eq "Address_of") && ($3 eq "Offset") && ($5 ne undef) ) 
           {
             ...
             if (exists $Address_List{$2})
             {
               ...
               if ($DODEBUG_OUT == 1)
               {
                   print "Handle: Address_of $2= $Address_List{$2} plus offset $4$5 \n";
               }
               $Zahl = hex("$Address_List{$2}");
