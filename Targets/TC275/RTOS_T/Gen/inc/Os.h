...
#define OS_TARGET_PORT "TriCoreTasking"
#define OS_TARGET_PORT_TriCoreTasking
#define OS_TARGET_MAJOR_VERSION (5U)
#define OS_TARGET_MINOR_VERSION (0U)
#define OS_TARGET_PATCH_VERSION (15U)
...
/* [MISRA 2012 Dir 4.9] */ /*lint -estring(9026, CAT1_ISR, CAT1_TRAP, OS_MAIN, OS_MFCR, OS_MTCR, OS_DISABLE, OS_ENABLE, OS_ISYNC, OS_DSYNC, OS_SYSCALL, OS_NOP, OS_SEQ_POINT, OS_DEBUG, OS_SWAP, OS_ALIGN, OS_ASM) */
/* CAT1_ISR, CAT1_TRAP, OS_MAIN, OS_MFCR, OS_MTCR, OS_DISABLE, OS_ENABLE, OS_ISYNC, OS_DSYNC, OS_SYSCALL, OS_NOP, OS_SEQ_POINT, OS_DEBUG, OS_SWAP, OS_ALIGN, OS_ASM */
#define OS_MAIN() extern void Os_main(void); void main(void) { OS_DSYNC(); OS_MTCR(0xfe04U, 0x200 | (OS_MFCR(0xfe04U))); /* Ensure PSW.IS */ Os_StartCoreGate(); Os_main(); } void Os_main(void)  /* [$Satisfies $TgtCore 56] */
...
/* Lint does not know about the compiler intrinsics and attributes so
   we supply equivalent descriptions here
 */
#ifdef _lint
 extern uint32 __mfcr(uint32 reg); /*lint -sem(__mfcr,pure) */
 extern void __mtcr(uint32 reg, uint32 val);
 extern void __enable(void);
 extern void __disable(void);
 extern void __isync(void);
 extern void __dsync(void);
 extern void __nop(void);
 extern void __syscall(uint32 val);
 #define OS_NOINLINE /* nothing */
 #define OS_SEQ_POINT()  /* nothing */
 #define OS_ALIGN(x) /* nothing */
 extern uint32 __swap(volatile uint32 *var, uint32 val);
 #define OS_SWAP(x)  __swap((volatile Os_Lockable)(x), 1U)
 #define OS_ASM(x) /* nothing */
#else
 #define OS_NOINLINE   __attribute__((noinline))
 #define OS_SEQ_POINT() __asm("")
 #define OS_ALIGN(x) __align(x)
 #define OS_SWAP(x)  __swap((unsigned int *)(&(x)), 1U)
 #define OS_ASM(x) __asm volatile(x)
#endif

#define OS_MFCR(x) ((uint32)__mfcr(x))
#define OS_MTCR(x, y) __mtcr(x,y)
#define OS_DISABLE() __disable()
#define OS_ENABLE() __enable()
#define OS_ISYNC() __isync()
#define OS_DSYNC() __dsync()
#define OS_DEBUG() __debug()
#define OS_NOP() __nop()
#define OS_SYSCALL(x) __syscall(x)
...
#ifndef _lint /* Lint can't see side effects */
/* [MISRA 2012 Rule 1.3] */ /*lint -save -esym(9023, MISRA 2012 Rule 1.3) */
/* [MISRA 2012 Rule 20.10] */ /*lint -estring(9024, CAT1_ISR) */
...
/*lint -restore */
/* [MISRA 2012 Rule 1.3] */ /*lint -save -esym(9023, MISRA 2012 Rule 1.3) */
/* [MISRA 2012 Rule 20.10] */ /*lint -estring(9024, CAT1_TRAP) */
#define CAT1_TRAP(x) void __trap(OS_TRAPVEC_##x) \
  __vector_table(0) \
  x(void)
/*lint -restore */
#endif
...
/* ------------------------------------------------- */
/* [MISRA 2012 Rule 20.1] */ /*lint -save -estring(9019, *) */
#define OS_START_SEC_CORE0_CODE_LIB
#include "Os_MemMap.h" /* [MISRA 2012 Dir 4.10] */ /*lint !e537 !e451 */
/*lint -restore */
extern FUNC(void, OS_CORE0_CODE) Os_ISRWrapper0(uint32 isr_index, uint32 pcxi);
/* [MISRA 2012 Rule 20.1] */ /*lint -save -estring(9019, *) */
#define OS_STOP_SEC_CORE0_CODE_LIB
#include "Os_MemMap.h" /* [MISRA 2012 Dir 4.10] */ /*lint !e537 !e451 */
/*lint -restore */
/* ------------------------------------------------- */
/* [MISRA 2012 Rule 20.1] */ /*lint -save -estring(9019, *) */
...
#define Os_SetRestartPoint() ( (OS_NOAPPMODE == Os_CurrentAppMode) ? (Os_setjmp(Os_AnyCoreInfo[OS_MFCR(0xfe1cU)].RestartJumpBuf), Os_AnyCoreInfo[OS_MFCR(0xfe1cU)].Restartable = TRUE, E_OK) : E_OS_SYS_NO_RESTART) /* [$UKS  770] [$UKS  771] */ /*lint !e545 Address of what might be an array */
/* [MISRA 2012 Dir 4.9] */ /*lint -estring(9026, StartOS) */
#define StartOS(x) {Os_StackBase[OS_MFCR(0xfe1cU)] = Os_GetSP(); if (Os_StartOS(x)) {while(Os_Cbk_Idle()) {} /* [$UKS 161] */; while((1)) {} /* [$UKS 16] */}}
...
#define GetCoreID() ((CoreIdType)OS_MFCR(0xfe1cU))  /* [$UKS 1619] [$UKS 1620] [$UKS 1621] */
