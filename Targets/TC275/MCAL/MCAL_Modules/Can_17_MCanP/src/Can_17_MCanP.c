...
**                                                                           **
**  $CC VERSION : \main\dev_tc23x_as4.0.3\122 $                              **
**                                                                           **
**  $DATE       : 2018-03-22 $                                               **
**                                                                           **
...
#if ( CAN_17_MCANP_SW_MAJOR_VERSION != 3U )
#error "CAN_17_MCANP_SW_MAJOR_VERSION does not match. "
#endif
#if ( CAN_17_MCANP_SW_MINOR_VERSION != 1U )
...
#ifdef IFX_CAN_DEBUG
extern volatile uint32 TestCan_DebugMask;
#endif
...
#define CAN_FD_INDICATOR_BIT        ((uint32)0x40000000U)
#else
#define CAN_FD_INDICATOR_BIT            ((uint16)0x4000U)
...
/*Max counter to exit from the loop(inside Can_lRxIsrExtractData() function),
  to avoid indefinite looping*/
#define CAN_EXTRACTDATA_LOOP_CNT          (6U)
...
#endif /* (CAN_TX_HARDWARE_OBJECTS_MAX > 0U) */
#endif
...
#if (CAN_TX_HARDWARE_OBJECTS_MAX > 0U)
IFX_LOCAL_INLINE Can_ReturnType Can_lWriteMsgObj(uint16 HthIndex,
                                             uint16 HwObjectId,
                                             const Can_PduType *PduInfo);
...
IFX_LOCAL_INLINE uint16 Can_lCalculateHrhId(uint8 KerIdx,uint16 HwObjectId);
#if(CAN_WAKEUP_CONFIGURED == STD_ON)
...
IFX_LOCAL_INLINE void Can_lRxIsrExtractData(uint8 HwControllerId,
                                        ...
                                        uint8 PndBitPosition);
#endif /*#if (CAN_DEV_ERROR_DETECT == STD_ON)*/
/* Read messages received from message buffer */
IFX_LOCAL_INLINE void Can_lReadMsgObject(uint8 HwControllerId,\
                                         uint16 HwObjectId,uint8 MessageLength);
...
#endif /*#if (CAN_DEV_ERROR_DETECT == STD_ON)*/
/* Read messages received from message buffer */
IFX_LOCAL_INLINE void Can_lReadMsgObjectRuntime(uint8 HwControllerId,\
                                         uint16 HwObjectId,uint8 MessageLength);
...
#define CAN_17_MCANP_START_SEC_VAR_INIT_8BIT
/*IFX_MISRA_RULE_19_01_STATUS=File inclusion after pre-processor directive is
 allowed only for MemMap.h*/
#include "MemMap.h"
...
#define CAN_17_MCANP_STOP_SEC_VAR_INIT_8BIT
/*IFX_MISRA_RULE_19_01_STATUS=File inclusion after pre-processor directive is
 allowed only for MemMap.h*/
#include "MemMap.h"
                                   ...
                                   const Can_PduType *PduInfo)
{
  ...
  if (E_OK == Status)
  {
    ...
    #else
    /* Since there is no TX objects are configured, passed Hth would be of
    type RX. Hence the DET is reported. */
    Det_ReportError(CAN_17_MCANP_MODULE_ID, CAN_17_MCANP_INSTANCE_ID,
      CAN_SID_WRITE, CAN_E_PARAM_HANDLE);
    /* Status is not initialised with E_NOT_OK here, since there is
    no functional code available in the else part of below if condition,
    incase of no TX objects. Return value also set in below else condition */
    #endif /* #if (CAN_TX_HARDWARE_OBJECTS_MAX > 0U) */
...
#if (CAN_RX_HARDWARE_OBJECTS_MAX > 0U)
    HthIndex = (uint16)(Hth - Can_kConfigPtr->CanNumRxHwObj);
#else
    HthIndex = (uint16)Hth;
#endif /* #if (CAN_RX_HARDWARE_OBJECTS_MAX > 0U) */
...
if(HwControllerId <= CAN_NUM_CONTROLLERS_IN_DEVICE)
{
    ...
    MsgIndex++)
  {
                    ...
                    CAN_MSID_NO_MSG_PND_VALUE) && (LoopBreak < CAN_THIRTY_TWO))
   {
      ...
      TxHandleIdx = Can_lGetHthIndex(KerIdx,TxMOIdx);
      ...
      CanIf_TxConfirmation((PduIdType)Can_SwObjectHandle[KerIdx][TxHandleIdx]);
...
IFX_LOCAL_INLINE Std_ReturnType Can_lInitClkSetting(void)
{
   ...
   (LoopBreak == CAN_ZERO)); KerIdx++)
  {
   ...
   else
   {
    ...
    /* Select fASYN_CAN or fERAY or External oscillator as the input source */
                                            ...
                                            uint8 HwControllerId)
{
  ...
  RxSrcNum =  (uint8)((CAN_NUM_CONTROLLERS_IN_KERNEL[KerIdx])
    + HwCtrlIDKer + IntSrcOffset);
/* Check whether controller is in Interrupt mode */
/*IFX_MISRA_RULE_17_04_STATUS=Pointer arithmetic used due to
        PBConfigStructure and is within allowed range.*/
  if (CAN_INTERRUPT ==
    Can_kConfigPtr->CanEventHandlingConfigPtr[ControllerId].\
  ...
  */
  ...
  else
  {
    ...
    */
    CAN_SFR_INIT_USER_MODE_MODIFY32(CAN_KER[KerIdx]->N[HwCtrlIDKer].CR.U,\
                CAN_NCR_CLEARMASK,(CAN_NCR_INIT |CAN_NCR_CCE ))
  }
#if (CAN_FD_ENABLE== STD_ON)
...
#endif
      {
        ...
        PBConfigStructure and is within allowed range.*/
          ...
          CAN_SFR_INIT_USER_MODE_WRITE32(CAN_KER[KerIdx]->MO[HwObjectId].AR.U,\
                         TxHwObjCfgPtr[HwObjIndex].TxObjPriorityClass);
...
**                    FrameType - Message Id Type (CAN_ID_MIXED not supported **
**                                for Fifo hardware objects)                  **
**                                                                            **
** Parameters (out) : None                                                    **
**                                                                            **
** Return value     : None                                                    **
...
static void Can_lClearReceivedMsg(uint8 HwControllerId)
{
    ...
    MsgIndex++)
  {
                    ...
                    && (LoopBreak32 < CAN_THIRTY_TWO))
    {
      ...
      /* Clear NEWDAT, RXPND and MSGLST bit */
      CAN_SFR_RUNTIME_USER_MODE_WRITE32(CAN_KER[KerIdx]->MO[HwObjectId].CTR.U,\
            CAN_MOSTAT_MSGLST_CLR | CAN_MOCTR_NEWDAT_CLR | CAN_MOCTR_RXPND_CLR);
...
static void Can_lClearReceivedMsgRuntime(uint8 HwControllerId)
{
  ...
  for(LoopIndex = CAN_ZERO;LoopIndex < RxObjectcount;LoopIndex++)
  {
              ...
              (CAN_KER[KerIdx]->MO[HwObjectId].STAT.U))& CAN_MOSTAT_RXPND_MASK)
    {
       /* Clear NEWDAT, RXPND and MSGLST bit */
       CAN_SFR_RUNTIME_USER_MODE_WRITE32(CAN_KER[KerIdx]->MO[HwObjectId].CTR.U,\
          (CAN_MOCTR_NEWDAT_CLR | CAN_MOCTR_RXPND_CLR | CAN_MOSTAT_MSGLST_CLR));
    }
                                             ...
                                             const Can_PduType *PduInfo)
{
...
#else
  CanCtrStat = (uint32)
  (CAN_SFR_RUNTIME_USER_MODE_READ32(CAN_KER[KerIdx]->MO \
  [HwObjectId].STAT.U) & \
  (CAN_MOCTR_RXUPD_CLR | CAN_MOCTR_TXPND_CLR |CAN_MOCTR_TXRQ_CLR));
#endif

  /* Check if hardware transmit object is busy with another transmit request
    and Check if a pre-emptive call of Can_Write has been issued, that cannot be
  ...
  if (CanCtrStat== CAN_ZERO )
  {
    ...
    if (PduInfo->id & CAN_EXTENDED_MSB_SET)
    {
      ...
      (unsigned_int) \
      (Can_kConfigPtr->CanTxHwObjectConfigPtr[HthIndex].TxObjPriorityClass |\
                                                      CAN_USE_EXTENDED_ID));
...
#if (CAN_FD_ENABLE == STD_ON)
    if (PduInfo->id & CAN_FD_INDICATOR_BIT)
    {
    ...
    /* Calculate the starting value of Hw Object Id */
  TxMOIdx = HwObjectId-(Can_kConfigPtr->CanTxHwObjCntKer[KerIdx]);
  /* Calculate the  value of Tx Handler index */
  TxHandleIdx = Can_lGetHthIndex(KerIdx,TxMOIdx);
  ...
  Can_SwObjectHandle[KerIdx][TxHandleIdx] = PduInfo->swPduHandle;
...
#endif
{
  ...
  uint8 KerIdx;
  uint16 Hrh = 0U;
#if((CAN_RX_HARDWARE_OBJECTS_MAX > 0U) &&  (CAN_STANDARD_ID_ONLY == STD_OFF))
  const Can_RxHwObjectConfigType *RxHwObjCfgPtr = \
                                        Can_kConfigPtr->CanRxHwObjectConfigPtr;
  #endif
  ...
  Status = E_OK;
  MessageId = CAN_ZERO;
#if ((CAN_FD_MSG_BYTES_LEN_SUPPORT == STD_ON) && (CAN_FD_ENABLE == STD_ON))
  MessageLength = CAN_ZERO;
#else
  MessageDlc = CAN_ZERO;
#endif
#endif
  ...
  {
#if (CAN_DEV_ERROR_DETECT == STD_ON)
    /*check for message object over-writing or data inconsistency.
      The condition (LoopBreak2 != CAN_ZERO) indicates data inconsistency, i.e.
      ...
      called, if DET enabled; The function is exited to avoid blocking.
         ...
         != CAN_ZERO) || (LoopBreak2 != CAN_ZERO))
    {
      ...
      Status = E_NOT_OK;
      /* set the loop break value to immediately exit from the loop */
      LoopBreak2 = CAN_EXTRACTDATA_LOOP_CNT + CAN_ONE;
    }
    else
#endif /* #if (CAN_DEV_ERROR_DETECT == STD_ON) */
    {
      /* Clear NEWDAT, RXPND and MSGLST */
      CAN_KER[KerIdx]->MO[HwObjectId].CTR.U =\
        (CAN_MOCTR_NEWDAT_CLR | CAN_MOCTR_RXPND_CLR | CAN_MOSTAT_MSGLST_CLR);
...
#if((CAN_RX_HARDWARE_OBJECTS_MAX > 0U) &&  (CAN_STANDARD_ID_ONLY == STD_OFF))
      /* check whether MO is in mixed mode */
     /*IFX_MISRA_RULE_17_04_STATUS=Pointer arithmetic used due to
     PBConfigStructure and is within allowed range.*/
      if(RxHwObjCfgPtr[Hrh].FrameType == CAN_ID_MIXED)
      {
        /* Re-initialize with the configured message id */
        /*IFX_MISRA_RULE_17_04_STATUS=Pointer arithmetic used due to
        PBConfigStructure and is within allowed range.*/
        CAN_KER[KerIdx]->MO[HwObjectId].AR.U =
             (unsigned_int)((CAN_PRIORITY_ID_BASED |CAN_USE_EXTENDED_ID |\
                 (RxHwObjCfgPtr[Hrh].MsgId)));
      }
#endif
#if (CAN_FD_ENABLE == STD_ON)
      if(CAN_KER[KerIdx]->MO[HwObjectId].FCR.U & CAN_FCR_FDF_MASK)
      {
        MessageId |= CAN_FD_INDICATOR_BIT;
      }
#endif
      /* Extract DLC */
      ...
      /* Message length varies from DLC when FD is enabled and it can go
      beyond 8. DLC of 9,10,11,12,13,14,15 corresponds to
      message length 12,16,20,24,32,48,64 respectively */
      ...
      /* Read messages received from message buffer */
      Can_lReadMsgObject(HwControllerId,HwObjectId,MessageLength);
      LoopBreak2++;
...
#ifdef IFX_CAN_DEBUG03
    TempMoctr = CAN_KER[KerIdx]->MO[HwObjectId].STAT.U | TestCan_DebugMask03;
#else
    ...
    &&
    (LoopBreak2 <= CAN_EXTRACTDATA_LOOP_CNT)));

#if (CAN_DEV_ERROR_DETECT == STD_ON)
  /*If MSGLST or Data inconsistency happend, DET reported and exit from function
    by discarding the data  */
  if(E_OK == Status)
#endif
  {
#if ((CAN_FD_MSG_BYTES_LEN_SUPPORT == STD_ON) && (CAN_FD_ENABLE == STD_ON))
    /* message length passed to CanIf should be DLC if FD_MSG_LENGTH support
    disabled  */
    MessageDlc = MessageLength;
#endif
#if (CAN_LPDU_RX_CALLOUT == STD_ON)
    /* Call receive L-PDU callout function if configured */
    RetVal = Can_kConfigPtr->Can_LPduRxCalloutFuncPtr((Can_HwHandleType)Hrh,
      MessageId,
      MessageDlc,
...
**                                                                            **
** Syntax           :IFX_LOCAL_INLINE void Can_lReadMsgObject\                **
**                          (uint8 HwControllerId,\                           **
**                              uint16 HwObjectId,uint8 MessageLength)        **
**                                                                            **
**                                                                            **
...
#endif
{
  ...
  /* Find the Hrh ID mapped to Hw MO Buffer */
  Hrh = Can_lCalculateHrhId(KerIdx,HwObjectId);
  do
  {
#if (CAN_DEV_ERROR_DETECT == STD_ON)
    ...
    if(((CAN_SFR_RUNTIME_USER_MODE_READ32(CAN_KER[KerIdx]->\
               MO[HwObjectId].STAT.U) & CAN_MOSTAT_MSGLST_CLR) != CAN_ZERO) ||
      (LoopBreak2 != CAN_ZERO))
    {
      /* Clear MSGLST, NEWDAT and RXPND */
       CAN_SFR_RUNTIME_USER_MODE_WRITE32(CAN_KER[KerIdx]->MO[HwObjectId].CTR.U,\
...
#endif /* #if (CAN_DEV_ERROR_DETECT == STD_ON) */
    {
      ...
      CAN_SFR_RUNTIME_USER_MODE_WRITE32(CAN_KER[KerIdx]->MO[HwObjectId].CTR.U,\
        ...
        /* Read messages received from message buffer */
        Can_lReadMsgObjectRuntime(HwControllerId,HwObjectId,MessageLength);
        LoopBreak2++;
...
** TRACEABILITY     : [cover parentID=]                                       **
**                                                                            **
** Syntax           :IFX_LOCAL_INLINE void Can_lReadMsgObjectRuntime          **
**                          (uint8 HwControllerId,\                           **
**                              uint16 HwObjectId,uint8 MessageLength)        **
**                                                                            **
**                                                                            **
...
** Syntax           :                                                         **
**                                                                            **
**      IFX_LOCAL_INLINE uint16 Can_lCalculateHrhId                           **
**                                     (                                      **
**                                       uint8 KerIdx,                        **
**                                       uint16 HwObjectId                    **
**                                     )                                      **
...
IFX_LOCAL_INLINE uint16 Can_lCalculateHrhId(uint8 KerIdx, uint16 HwObjectId)
{
  ...
  /* Calculate the starting value of HW Object Id */
#if ((CAN_RX_OBJECT_KER0 > 0U) || (CAN_RX_OBJECT_KER1 > 0U))
  HrhOffset = HwObjectId - RxObjOffset;
...
** Reentrancy       : Reentrant                                               **
**                                                                            **
** Parameters (in)  : FifoTopBot - FifoTop & FifoBottom values                **
**                    CurrentMo  - Associated Message Object                  **
**                    HwControllerId - Hardware Controller ID                 **
                                         ...
                                         uint8 Threshold)
{
  ...
  {
    #if ( CAN_DEV_ERROR_DETECT == STD_ON)
    if ((CAN_KER[KerIdx]->MO[CurrentMo].STAT.U)& CAN_MOSTAT_MSGLST_CLR)
    {
      /*Clear MSGLOST*/
      CAN_KER[KerIdx]->MO[CurrentMo].CTR.U = CAN_MOSTAT_MSGLST_CLR;

      /* Report to DET */
      Det_ReportError(CAN_17_MCANP_MODULE_ID, CAN_17_MCANP_INSTANCE_ID,\
        CAN_SID_RECEIVEISR, CAN_E_RXFIFO_DATALOST);
    }
    #endif
    locMoDataIndex3= CAN_ZERO;

    /* Clear NEWDAT, RXPND and MSGLST(should be cleared for DET OFF case) */
    CAN_KER[KerIdx]->MO[CurrentMo].CTR.U =\
           (CAN_MOCTR_NEWDAT_CLR | CAN_MOCTR_RXPND_CLR | CAN_MOSTAT_MSGLST_CLR);
