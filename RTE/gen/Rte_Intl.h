/** @file         Rte_Intl.h
  ...
  *
  * Template:     Id: Rte_Intl_h.template 87318 2019-02-01 10:01:02Z DIR9YOK
  */
...
extern VAR(Rte_ResourceRefType, RTE_DATA) Rte_Resources[4];
...
#include "Rte_MemMap.h" /*lint !e537 permit multiple inclusion */
/* [$Satisfies $SRD 97814] Runtime selection of resource for the current OsApplication */
extern VAR(Rte_ResourceCountRefType, RTE_DATA) Rte_ResourceCount[4];
#define RTE_SYS_STOP_SEC_VAR_CLEARED_32
#include "Rte_MemMap.h" /*lint !e537 permit multiple inclusion */
...
/* [$Satisfies $SRD 97814] Runtime selection of resource for the current OsApplication */
#define Rte_GetResource() { ApplicationType osappID = GetApplicationID(); if ( !RTE_COUNTER_FROM_REF( Rte_ResourceCount[osappID] ) ) { (void)GetResource( RTE_RESOURCE_FROM_REF( Rte_Resources[osappID] ) ); } /* resource locked */ ++RTE_COUNTER_FROM_REF(Rte_ResourceCount[osappID]); }
#endif
#ifndef Rte_ReleaseResource
/* [$Satisfies $SRD 97814] Runtime selection of resource for the current OsApplication */
#define Rte_ReleaseResource() { ApplicationType osappID = GetApplicationID(); if ( 0u == --RTE_COUNTER_FROM_REF( Rte_ResourceCount[osappID] ) ) { (void)ReleaseResource( RTE_RESOURCE_FROM_REF( Rte_Resources[osappID] ) ); } }
#endif
