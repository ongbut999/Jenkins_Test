/** @file     SchM_ComM.h
  *
  * @brief    Basic Software Scheduler Module Interlink header file
  *
  * @note     AUTOMATICALLY GENERATED FILE! DO NOT EDIT!
  *
  * @note     Generated by ETAS RTA-RTE
  * @note     Version 6.8.0 
  */

#ifndef SchM_ComM
#define SchM_ComM

#include "SchM_ComM_Type.h"
#include "Rte_Intl.h"
#ifdef __cplusplus
extern "C" {
#endif /* __cplusplus */

#if defined(RTE_RUNNABLEAPI_BSWSE_MainFunction_ComMChannel_Can_Network_Channel)
   /* Do nothing for this and all elif clauses below (rte_sws_2751) */
#elif defined(RTE_RUNNABLEAPI_BSWSE_MainFunction_ComMChannel_Can_Network_XCP_Channel)
#else
   /* None of above defined so define value including all APIs (rte_sws_2751) */
   #define RTE_PRV_ALL_API
#endif

/* API Mapping Macros */
#ifndef RTE_CORE

#endif /* RTE_CORE */

/*******************************************************
 ***
 *** Schedulable Entity Prototypes
 ***
 *******************************************************/

#define COMM_START_SEC_CODE
#include "ComM_MemMap.h" /*lint !e537 permit multiple inclusion */
FUNC(void, COMM_CODE) ComM_MainFunction_ComMChannel_Can_Network_Channel(void);
#define COMM_STOP_SEC_CODE
#include "ComM_MemMap.h" /*lint !e537 permit multiple inclusion */
#define COMM_START_SEC_CODE
#include "ComM_MemMap.h" /*lint !e537 permit multiple inclusion */
FUNC(void, COMM_CODE) ComM_MainFunction_ComMChannel_Can_Network_XCP_Channel(void);
#define COMM_STOP_SEC_CODE
#include "ComM_MemMap.h" /*lint !e537 permit multiple inclusion */

#ifdef __cplusplus
} /* extern C */
#endif /* __cplusplus */

#endif /* !SchM_ComM */
